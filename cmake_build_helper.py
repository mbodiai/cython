...
def find_python_library():
    ...
    if lib_name:
        ...
        if sys.platform == 'darwin' and 'Python.framework' in lib_name:
             ...
             if framework_dir and framework_dir != 'no-framework':
                  ...
                  if shared_lib: return shared_lib
    ...
    try:
        ...
        if found_lib:
            ...
            if found_lib_path: return found_lib_path
    except Exception:
    ...
    if sys.platform == "darwin":
        try:
            output = subprocess.check_output(["otool", "-L", sys.executable], text=True, stderr=subprocess.DEVNULL)
    ...
    # 4. Linux: Use `ldd` (carefully)
        ...
        try:
            output = subprocess.check_output(["ldd", sys.executable], text=True, stderr=subprocess.DEVNULL)
            for line in output.split("\n"):
                line = line.strip()
    ...
    # 5. Windows: Search for pythonXX.dll
        ...
        potential_paths = [
            ...
            Path(os.environ.get("SystemRoot", "C:\\Windows")) / "System32" / f"python{python_version.major}{python_version.minor}.dll",
            Path(os.environ.get("SystemRoot", "C:\\Windows")) / "SysWOW64" / f"python{python_version.major}{python_version.minor}.dll",
        ]
        for path in potential_paths:
            dll_path = check_path(path)
            if dll_path: return dll_path
    ...
    if lib_name:
        common_paths = [
            ...
            Path("/lib"), Path("/lib64"), Path("/usr/lib64"),
        ]
        ...
        for path in common_paths:
            if path.is_dir():
                ...
                patterns = [f"libpython{sys.version_info.major}.{sys.version_info.minor}*.so*",
...
def run_cython(source_file, output_dir, embed=False, language='c'):
    ...
    from Cython.Utils import replace_suffix # Need this utility
...
if __name__ == "__main__":
    ...
    parser.add_argument("--find-python-lib", action="store_true", help="Find and print absolute path to Python library")
    parser.add_argument("--find-python-include", action="store_true", help="Find and print Python include path")
    ...
    parser.add_argument("--language", choices=['c', 'c++'], default='c', help="Output language for Cython (c or c++)")
    ...
    if args.generate_main:
         if not args.install_dir or not args.module_name:
              print("ERROR: --install-dir and --module-name are required with --generate-main", file=sys.stderr)
              sys.exit(1)
