...
class PyStringConst:
    ...
    # intern      boolean
    # is_unicode  boolean
    # index       int     index in __pyx_string_tab
...
class GlobalState:
    ...
    def generate_string_constants(self):
        ...
        py_unicode_consts.sort(key=lambda item: (item[0].index if item[0].index is not None else 1 << 30, item[0].cname))
        for idx, (py_string, _) in enumerate(py_unicode_consts):
            if py_string.index is None:
                py_string.index = idx

        start_index = len(py_unicode_consts)
        # Preserve deterministic ordering for bytes constants as well.
        ...
        # Repackage for downstream generation (expected tuple shape).
        py_unicode_consts = [
