...
from ..Compiler import Directives, Pipeline
from ..Compiler.Main import Context
from ..Compiler.Options import DEFAULT_COMPILATION_OPTIONS, get_directive_defaults
from ..Compiler.ParseTreeTransforms import SkipDeclarations
from ..Compiler.TreeFragment import parse_from_strings
from ..Compiler.Visitor import EnvTransform
from ..Utility.ImportUtils import smart_import
...
_INLINE_HELPER_MODULES = {'Cython.Build.Inline', 'Cython.Shadow'}


def _resolve_namespace(locals_dict, globals_dict):
    if locals_dict is not None and globals_dict is not None:
        return locals_dict, globals_dict

    frame = inspect.currentframe()
    try:
        frame = frame.f_back
        while frame and frame.f_globals.get('__name__') in _INLINE_HELPER_MODULES:
            frame = frame.f_back
        if frame is None:
            return locals_dict or {}, globals_dict or {}
...
def _populate_unbound(kwds, unbound_symbols, locals=None, globals=None):
    locals, globals = _resolve_namespace(locals, globals)
...
def _cython_inline(code, get_type=unsafe_type,
    ...
    cython_compiler_directives = (
        cython_compiler_directives.copy()
        if cython_compiler_directives
        else get_directive_defaults().copy()
    )
    ...
    return locals()
        ...
        if sys.platform == 'win32' and sys.version_info >= (3, 8):
            with os.add_dll_directory(str(Path(lib_dir).resolve())):
                module = load_dynamic(module_name, str(module_path))
        else:
            module = load_dynamic(module_name, str(module_path))
...
class CythonInline:
    ...
    def __call__(self,*args, **kwargs):
        locals_arg = kwargs.pop('locals', None)
        globals_arg = kwargs.pop('globals', None)
        if locals_arg is None or globals_arg is None:
            ...
            try:
                ...
                del frame
        return _cython_inline(*args, locals=locals_arg, globals=globals_arg, **kwargs)

cython_inline = CythonInline()
_ALLOWED_DIRECTIVE_KEYS = frozenset(get_directive_defaults())
