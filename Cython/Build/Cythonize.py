...
def _build(ext_modules, parallel):
    ...
    with _interruptible_pool(pool_cm) as proc_pool:
        ...
        if worker_exceptions:
            ...
            non_base_exceptions = [exc for exc in worker_exceptions if isinstance(exc, Exception)]
...
def benchmark(code, setup_code=None, import_module=None, directives=None):
    ...
    print(
        f"{number} loops, best of {len(timings)}: {format_time(fastest)} per loop (median: {format_time(median)})"
    )
...
Environment variables:
  ...
  https://setuptools.pypa.io/en/latest/userguide/ext_modules.html#compiler-and-linker-options""",
    ...
    parser.add_argument(
        "-f", "--force", dest="force", action="store_true", default=None, help="force recompilation"
    )
    parser.add_argument(
        "-q",
        "--quiet",
