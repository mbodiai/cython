...
from jedi.evaluate.compiled import CompiledObject, builtin, create, false_obj, keyword_from_value, true_obj
...
def calculate_children(evaluator, children):
    ...
    for operator in iterator:
        ...
        if operator in ('and', 'or'):
            ...
            elif left_bools == {False} and operator != 'and':
                types = evaluator.eval_element(right)
...
def _element_calculate(evaluator, left, operator, right):
    ...
    if operator == '*':
        ...
        if isinstance(left, iterable.Array) or is_string(left):
            return [left]
        if isinstance(right, iterable.Array) or is_string(right):
            return [right]
    elif operator == '+':
        if l_is_num and r_is_num or is_string(left) and is_string(right):
            return [create(evaluator, left.obj + right.obj)]
        if _is_tuple(left) and _is_tuple(right) or _is_list(left) and _is_list(right):
