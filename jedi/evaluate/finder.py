...
from jedi.evaluate import analysis, compiled, docstrings, dynamic, flow_analysis, helpers, imports, iterable, param
    ...
    def find(self, scopes, search_global=False):
        ...
        if not names and not types \
            ...
            if not isinstance(self.name_str, str | unicode):  # TODO Remove?
                if search_global:
                    message = (f"NameError: name '{self.name_str}' is not defined.")
...
def _name_to_types(evaluator, name, scope):
    ...
    if typ.isinstance(tree.ForStmt) or typ.isinstance(tree.CompFor):
...
def _eval_param(evaluator, param, scope):
    ...
    from jedi.evaluate.param import Arguments, ExecutedParam
    ...
    if not res_new and param.stars:
