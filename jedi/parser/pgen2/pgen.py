...
class ParserGenerator:
        ...
        for label, itsfirst in overlapcheck.items():
            for symbol in itsfirst:
                if symbol in inverse:
                    raise ValueError(f"rule {name} is ambiguous; {symbol} is in the"
    ...
    def dump_nfa(self, name, start, finish) -> None:
        todo = [start]
        for _i, state in enumerate(todo):
            for label, next in state.arcs:
                if next in todo:
                    todo.index(next)
                else:
    ...
    def dump_dfa(self, name, dfa) -> None:
        for _i, state in enumerate(dfa):
            for _label, _next in state.arcs.items():
                pass
    ...
    def raise_error(self, msg, *args) -> NoReturn:
        if args:
            try:
                msg = msg % args
            except:
...
class NFAState:
    ...
    def addarc(self, next, label=None) -> None:
