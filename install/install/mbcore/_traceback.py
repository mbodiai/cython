...
def first(*args, **kwargs):
    ...
    """
    p, *args = pull_out(callable, chain(*args, kwargs.values()))
    pred: Callable = next(iter(p), None)
    if pred is None:
        return next(iter(args), None)

    return next(filter(pred, collapse(args)), None)
...
def is_third_party(module_name_or_filepath: str | Path, package: str) -> bool:
    if module_name_or_filepath == "__main__":
        return False
    if isinstance(module_name_or_filepath, Path) or Path(module_name_or_filepath).exists():
        if "site-packages" in str(Path(module_name_or_filepath).resolve()):
            return True
        fp = Path(str(module_name_or_filepath)).resolve()
        candidate = fp.stem
    else:
        candidate = module_name_or_filepath
        fp = None
    if not isinstance(candidate, ModuleType):
        m = inspect.getmodulename(module_name_or_filepath) or candidate
        candidate = m
    if not isinstance(candidate, str):
        candidate = getattr(candidate, "__module__", getattr(candidate, "__name__", None))
    if candidate is None:
        return False

    return package not in candidate
...
def _link_fp_markup(line: str | Any, fn: str | Path, lineno: int | str, showpath=True, style=None) -> Text | str:
    ...
    lineno = int(str(lineno).strip()) if not isinstance(lineno, int) and str(lineno).strip().isnumeric() else lineno
    uri = get_uri()
    ...
    return f"[{style}]{line}[link={file_url}]{display_text}[/link][/{style}]"


def get_uri():
    if any("cursor" in v.lower() for v in os.environ.values()):
        return "cursor://file//"
    return "vscode://file//" if any("VSCODE" in name for name in os.environ) else "file://"


def link_fp(line, fn, lineno, markup=True, showpath=True, style=None) -> str | Text:
    """Create a clickable file link with line number."""
    if not fn:
        return line
    ...
    if markup:
        return _link_fp_markup(line, fn, lineno, showpath=showpath, style=style)
    ...
    style = style or ("bold grey39 underline" if not showpath else "bold blue underline")
    return f"[{style}]{line}\n[link={file_url}]{display_text}[/link][/{style}]"
    # return f"[link=file://{str(fn) + ':' + str(lineno)}]" + str(line) + "[/link]" + "\n" + f"{fn}:{lineno}"


def link_fp_str(line, fn, lineno, style=None, showpath=False) -> str | Text:
    return link_fp(line, fn, lineno, markup=True, style=style, showpath=showpath)


def safe_text(*strs: str | Text, ident="") -> Text:
    ...
    for s in strs:
        ...
        if isinstance(s, dict):
            s = ident + ",\n".join([f"{k}: {safe_text(v, ident + ' ')}" for k, v in s.items()])
        else:
            s = ident + str(s)

        if "\x1b[" in s:  # ANSI escape sequence
...
def install(
    *,
    ...
    indent_guides: bool = False,
    excluded_modules: Iterable[str | ModuleType] = (),
    max_frames: int = 100,
    ignore_third_party: bool = False,
    ...
    Args:
        ...
        excluded_modules (Sequence[Union[str, ModuleType]]): Optional sequence of modules or paths to exclude from traceback.
...
class Traceback:
    ...
    def __init__(
        ...
        *,
        ...
        code_width = code_width or width - 24
        extra_lines = extra_lines or 3
        if trace is None:
        ...
        self.excluded_modules: Sequence[str] = []
        for excluded_modules_entity in excluded_modules:
            if not isinstance(excluded_modules_entity, str):
                assert excluded_modules_entity.__file__ is not None, (
                    f"{excluded_modules_entity!r} must be a module with '__file__' attribute"
                )
                path = os.path.dirname(excluded_modules_entity.__file__)
            else:
                path = excluded_modules_entity
            path = os.path.normpath(os.path.abspath(path))
            self.excluded_modules.append(path)
    ...
    def from_exception(
        ...
        *,
        ...
        Args:
            ...
            locals_hide_dunder (bool, optional): Hide locals prefixed with double underscore. Defaults to True.
            locals_hide_sunder (bool, optional): Hide locals prefixed with single underscore. Defaults to False.
            excluded_modules (Iterable[Union[str, ModuleType]]): Optional sequence of modules or paths to exclude from traceback.
            max_frames (int): Maximum number of frames to show in a traceback, 0 for no maximum. Defaults to 100.
    ...
    def extract(
        ...
        *,
        ...
        if hasattr(exc_value, "depth"):
            depth = exc_value.depth

            def walker():
                yield from walk_tb(traceback)

            while depth > 0:
                f, l = next(walker())
                depth -= 1
        else:
            walker = walk_tb(traceback)
        ...
        while True:
            ...
            for frame_summary, line_no in walker:
                ...
                while (
                    not Path(f + ".py").exists()
                    and not Path(f + ".pyx").exists()
                    and not Path(f + ".pxd").exists()
                    and not Path(f + ".pxi").exists()
                    and "." in f
                ):
                    f = ".".join(f.split(".")[:-1])
                ...
                filename = (
                    ...
                    if Path(f + ".pyx").exists()
                    else f + ".pxd"
                    if Path(f + ".pxd").exists()
                    else f + ".pxi"
                    if Path(f + ".pxi").exists()
                    else f
                )
                ...
                frame = Frame(
                    ...
                    module=frame_summary.f_globals.get("__module__", frame_summary.f_globals.get("__name__", "?")),
                    locals=(
                        {
                            key: pretty.traverse(
                                value,
                        ...
                        if show_locals or os.getenv("SHOW_LOCALS")
                        else None
                    ),
            ...
            if cause and not getattr(exc_value, "__excluded_modules_context__", False):
                exc_type = cause.__class__
                exc_value = cause
                traceback = cause.__traceback__
                is_cause = False
        ...
        for last, stack in loop_last(reversed(self.trace.stacks)):
            if stack.frames:
                t = link_fp(
                    ...
                    globals().get("__file__", Path(__file__).name),
                    517,
                    showpath=False,
                )
    ...
    def _render_stack(self, stack: Stack) -> RenderResult:
        ...
        excluded = False
        mod = first_true(
            pred=isnot("__main__"), coll=(getattr(f, "module", getattr(f, "name", "")) for f in stack.frames),
        )
        for frame_index, frame in enumerate(stack.frames):
            ...
            if is_third_party(getattr(frame, "module", getattr(frame, "__module__", "")), mod) and (
                self.ignore_third_party or os.getenv("TRACEBACK_IGNORE_THIRD_PARTY")
            ):
                continue
            ...
            try:
                ...
                f = cast(FrameType, frame)
                frame = Frame(
                    f.f_code.co_filename,
                    f.f_lineno,
                    f.f_code.co_name,
                    f.f_globals.get("__module__", f.f_globals.get("__name__", "?")),
                    locals=f.f_locals,
                )
            frame_filename = frame.filename
            excluded_modules = any(frame_filename.startswith(path) for path in self.excluded_modules)
