...
async def test_producer_consumer(deque: AsyncDeque):
    ...
    producers = [producer_left(deque), producer_right(deque)]

    consumers = [consumer(deque, total_items // 2), consumer(deque, total_items // 2)]

    # Run everything concurrently with timeout
    try:
        await asyncio.wait_for(asyncio.gather(*producers, *consumers, return_exceptions=True), timeout=10.0)
    except asyncio.TimeoutError:
