...
# from Cython.Compiler.CythonScope import CythonScope, create_cython_scope
...
#     found = ctypes.util.find_library(f"python{sys.version_info[0]}.{sys.version_info[1]}")
#     if found:
...
#         os.path.join(base_prefix, "include", f"python{sys.version_info.major}.{sys.version_info.minor}"),
#     )
#     cfg["LIBDIR1"] = validate_path(os.path.join(base_prefix, "lib"))
#     cfg["LIBDIR2"] = validate_path(
#         os.path.join(
#             base_prefix, "lib", f"python{sys.version_info.major}.{sys.version_info.minor}", f"config-{sys.platform}",
#         ),
#     )
...
#             raise FileNotFoundError(f"Resolved path '{resolved}' does not exist for '{libpath}'.")
#         return str(resolved)
#     return libpath
...
#     print("  INCDIR:", sysconfig.get_path("include"))
#     print("  LDLIBRARY (PYLIB):", sysconfig.get_config_var("LIBRARY"))
#     print("  LDLIBRARY (PYLIB_DYN):", sysconfig.get_config_var("LDLIBRARY"))
#     resolved = find_python_library()
#     print("Resolved Python library:", resolved)


# def _parse_args(args) -> tuple | None:
#     """Parse command-line arguments.

#     If no non-option argument is provided, return None to trigger configuration dump.
#     """
#     cy_args = []
#     last_arg = None
...
#     else:
#         raise FileNotFoundError(f"Neither {basename + '.cpp'} nor {basename + '.c'} exists.")
#     # Compile the source file into an object file.
#     cmd = [compiler, "-c", "-o", basename + ".o", source_file, "-I" + INCDIR] + CFLAGS.split()
...
#     def __init__(self, defaults: CompilationKwargs | None = None, **kw: Unpack[CompilationKwargs]):
#         self.include_path = []
#         if defaults:
#             if isinstance(defaults, CompilationOptions):
#                 defaults = cast(CompilationKwargs, defaults.__dict__)
#         else:
...
#                 "s" if len(unknown_options) > 1 else "", ", ".join(unknown_options),
#             )
#             raise ValueError(message)

#         directive_defaults = get_directive_defaults()
#         directives = options["compiler_directives"]
#         # check for invalid directives
#         unknown_directives = set(directives) - set(directive_defaults)
#         if unknown_directives:
#             message = "got unknown compiler directive{}: {}".format(
#                 "s" if len(unknown_directives) > 1 else "", ", ".join(unknown_directives),
#             )
#             raise ValueError(message)
...
#     def configure_language_defaults(self, source_extension: str):
...
#                 continue
#             if key in ["use_listing_file", "generate_pxi", "annotate", "annotate_coverage_xml"]:
...
# class CompilationKwargs(TypedDict):
...
#     modules: dict[str, BuiltinScope | ModuleScope] = Field(default_factory=dict)
...
#         self.cython_scope = create_cython_scope(self)
...
#     def process_pxd(self, source_desc: FileSourceDescriptor, scope: ModuleScope, module_name: str) -> tuple | None:
...
#             warning(
#                 pos,
#                 "Dotted filenames ('%s') are deprecated. Please use the normal Python package directory layout."
#                 % pxd_filename,
#                 level=1,
#             )
...
#         self,
#         module_name: str,
#         from_module: CythonScope | None = None,
#         pos: tuple[str, int, int] | None = None,
#         need_pxd: bool = True,
#         absolute_fallback: bool = True,
#         relative_import: bool = False,
#     ) -> CythonScope | None:
#         debug_find_module = 0
#         if debug_find_module:
#             print(
#                 f"Context.find_module: module_name = {module_name}, from_module = {from_module}, pos = {pos}, need_pxd = {need_pxd}",
#             )
#         scope = None
#         pxd_pathname = None
#         if from_module:
...
#             raise CompileError(pos or (module_name, 0, 0), f"'{module_name}' is not a valid module name")
#         if from_module:
#             if debug_find_module:
#                 print("...trying relative import")
#             scope = cast(ModuleScope, from_module.lookup_submodule(module_name))
#             if not scope:
...
#             "Decoding error, missing or incorrect coding=<encoding-name> at top of source (cannot decode with encoding %r: %s)"
#             % (encoding, msg),
#         )
...
# def run_pipeline(source: str, options: CompilationOptions, full_module_name=None, context=None) -> CompilationResult:
#     from Cython.Compiler import Pipeline
...
#     result = create_default_resultobj(CompilationSource(source_desc, full_module_name, cwd), options)
...
# def compile_single(source: str, options: CompilationOptions, full_module_name=None) -> CompilationResult:
#     return run_pipeline(source, options, full_module_name)


# def compile_multiple(sources: list[str], options: CompilationOptions) -> CompilationResultSet:
...
# def compile(
#     source: str | list[str], options: CompilationOptions, full_module_name=None, **kwds,
# ) -> CompilationResult | CompilationResultSet:
#     options = CompilationOptions(defaults=options, **kwds)
#     print(f"compile: source={source}, options={options}, full_module_name={full_module_name}")
...
# def search_include_directories(
#     dirs: list[str], qualified_name: str, suffix: str, pos: tuple, include: bool, source_file_path: str,
# ) -> str | None:
...
# @Utils.cached_function
# def search_module_in_dir(package_dir: str, module_name: str, suffix: str) -> str | None:
...
#                 raise
#             safe_print(f"{sys.argv[0]}: No such file or directory: '{e.filename}'", file=sys.stderr)
#             sys.exit(1)
#     else:
...
#     if parsed is None:
#         print("Usage: python build_executable.py [cython_options] input_file [runtime_args]")
#         print("Example: python build_executable.py hello.py")
#         print("\nThis tool compiles Python files to standalone executables using Cython")
#         print("\nNo input file specified. Configuration:")
#         dump_config()
#         sys.exit(1)
...
#     args = sys.argv[1:]
#     try:
#         program_name, remaining = _build(args)
#         if program_name and remaining:
