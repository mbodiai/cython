...
from importlib import abc, import_module
from importlib.machinery import FileFinder
from pathlib import Path

from typing_extensions import TYPE_CHECKING, NamedTuple
...
def resolve_name(name: str) -> "ModuleType | None":
    from mbcore.import_utils import smart_import

    return smart_import(name)


def module_package(path: Path) -> "tuple[ModuleType | None, str | None]":
    ...
    if stem in sys.modules:
        return sys.modules[stem], getattr(sys.modules[stem], "__package__", sys.modules[stem].__name__)

    pkg_path = path.parent
    while (pkg_path / "__init__.py").exists():
        pkg_path = pkg_path.parent

    path_string = (
        str(path.resolve().relative_to(pkg_path.resolve()))
        .replace("/", ".")
        .replace("\\", ".")
        .removesuffix(".py")
        .removeprefix(".")
    ...
    # Try importing different combinations of package/module
    for i in range(len(parts)):
        module_name = ".".join(parts[i:])
        package = ".".join(parts[:i]) or None  # Package can be None for top-level
    ...
    if not mod:
        spec = spec_from_file_location(maybe_mod, path)
        ...
        try:
            if spec is not None:
                try:
                    ...
                    module = import_module(spec.name, package=package)
            ...
            if verbose():
                ...
                traceback.print_exc()
            verbose("Failed to import module from path", path)
        if mod is None and spec is not None:
            try:
...
class ModuleInfo(NamedTuple):
    importer: "FileFinder | None"
    name: str
    ispkg: bool

    def __repr__(self) -> str:
        return f"ModuleInfo(importer={(self.importer.path[:10] + '...' + self.importer.path[-10:]) if self.importer else ''}, name={self.name}, ispkg={self.ispkg})"


def iter_modules(path: "list[str] | None" = None, prefix: str = "", third_party: bool = True) -> "Iterator[ModuleInfo]":
    ...
    """  # noqa: D401
    if path is None:
        importers = iter_importers(third_party=third_party)
    elif isinstance(path, str):
        raise ValueError(f"path must be None or list of paths to look for modules in. Got {path}")
    else:
    ...
    from mbcore.tree import collapse

    site_packages = collapse(getsitepackages())
    for i in importers:
        for name, ispkg in filter(lambda x: third_party or x[0] not in site_packages, iter_importer_modules(i, prefix)):
...
@singledispatch
def iter_importer_modules(importer: "Any", prefix: str = "") -> "list[ModuleInfo]":
...
IsPackage: "TypeAlias" = bool


# Implement a file walker for the normal importlib path hook
def _iter_file_finder_modules(importer: "FileFinder", prefix="") -> "Iterator[Tuple[ModuleName, IsPackage]]":
    if importer.path is None or not Path(importer.path).is_dir():
        return
    ...
    for fn in filenames:
        ...
        path = Path(importer.path) / fn
        ispkg = False

        if not modname and Path(path).is_dir() and "." not in fn:
            modname = fn
            try:
            ...
            for f in dircontents:
                subname = inspect.getmodulename(f)
                if subname == "__init__":
                    ispkg = True
                    break
            else:
...
iter_importer_modules.register(FileFinder, _iter_file_finder_modules)
...
try:
    ...
    def iter_zipimport_modules(importer, prefix=""):
        dirlist: list[str] = sorted(zipimport._zip_directory_cache[importer.archive])
        ...
        for fn in dirlist:
            ...
            f = fn[plen:].split(os.sep)

            if len(f) == 2 and f[1].startswith("__init__.py") and f[0] not in yielded:
                yielded[f[0]] = 1
                yield prefix + f[0], True

            if len(f) != 1:
                continue

            modname = inspect.getmodulename(f[0])
            if modname == "__init__":
                continue
...
def get_importer(path_item: "Any") -> "Any":
...
def iter_importers(fullname: str = "", third_party: bool = True):
    ...
    """
    if not third_party and ("test" in fullname or "site-packages" in fullname):
        return
    ...
    if "." in fullname:
        ...
        yield from filter(lambda x: third_party or "site-packages" not in str(x), sys.meta_path)
        path = sys.path
    for item in path:
        if not third_party and ("test" in item or "site-packages" in item):
            continue
        yield get_importer(item)


def get_loader(module_or_name: "Any") -> "Any":
    ...
    warnings._deprecated(
        "pkgutil.get_loader", f"{warnings._DEPRECATED_MSG}; use importlib.util.find_spec() instead", remove=(3, 14),
    )
...
def find_loader(fullname: str) -> "abc.Loader | None":
    ...
    """
    warnings._deprecated(
        "pkgutil.find_loader", f"{warnings._DEPRECATED_MSG}; use importlib.util.find_spec() instead", remove=(3, 14),
    )
    if fullname.startswith("."):
        msg = f"Relative module name {fullname!r} not supported"
        raise ImportError(msg)
...
def extend_path(path: list[str], name: str) -> list[str]:
    ...
    for dir in search_path:
        ...
        if finder is not None:
            ...
            elif finder is not None and hasattr(finder, "find_loader"):
...
def libc_ver(executable=None, lib="", version="", chunksize=16384):
    """Try to determine the version of libc used to build a Python executable.
    ...
    with open(executable, "rb") as f:
        ...
        while pos < len(binary):
            m = libc_search.search(binary, pos) if b"libc" in binary or b"GLIBC" in binary else None
...
def _norm_version(version, build=""):
    """Normalize the version and build strings.
...
def _syscmd_ver(system="", release="", version="", supported_platforms=("win32", "win16", "dos")):
    """Figure out the OS version used and returns a tuple (system, release, version).
    ...
    for cmd in ("ver", "command /c ver", "cmd /c ver"):
        try:
            info = subprocess.check_output(
                cmd, stdin=subprocess.DEVNULL, stderr=subprocess.DEVNULL, text=True, encoding="locale", shell=True,
            )
...
def _win32_ver(version, csd, ptype):
    ...
    try:
        ...
        csd = f"SP{spmajor}.{spminor}" if spminor and spminor != "0" else f"SP{spmajor}"
...
def system_alias(system, release, version):
    """Return (system, release, version) aliased to common marketing names used for some systems.
...
def _follow_symlinks(filepath):
    """In case filepath is a symlink, follow it until a real file is found."""
...
def architecture(executable=sys.executable, bits="", linkage=""):
    """Query the given executable (defaults to the Python interpreter binary) for various architecture information.
    ...
    fileout = _syscmd_file(executable, "") if executable else ""
    ...
    if "ELF" in fileout:
        ...
        linkage = "WindowsPE" if "Windows" in fileout else "PE"
...
class uname_result(NamedTuple):
    """A uname_result that's largely compatible with a simple namedtuple."""
...
def uname() -> uname_result:
    ...
    processor = _unknown_as_blank(_Processor.get())
...
def _sys_version(sys_version=None):
    ...
    # Try the cache first
    result = _sys_version_cache.get(sys_version)
    ...
    if sys.platform.startswith("java"):
        ...
        if match is None:
            raise ValueError(f"failed to parse Jython sys.version: {repr(sys_version)}")
        ...
        if match is None:
            raise ValueError(f"failed to parse PyPy sys.version: {repr(sys_version)}")
        ...
        match = sys_version_parser.match(sys_version)
        if match is None:
            raise ValueError(f"failed to parse CPython sys.version: {repr(sys_version)}")
...
def python_build():
    """Return a tuple (buildno, builddate) stating the Python build number and date as strings."""
    return _sys_version()[4:6]


def python_compiler():
    """Return a string identifying the compiler used for compiling Python."""
...
def platform(aliased=False, terse=False):
    ...
    # to it...
    system, node, release, version, machine, processor = uname_result(*uname())
    ...
    if system == "Windows":
        ...
        platform = _platform(system, release) if terse else _platform(system, release, version, csd)
...
if __name__ == "__main__":
    ...
    print(platform(aliased=aliased, terse=terse))

    print(ReprList(iter_modules(third_party=False)))
    sys.exit(0)
