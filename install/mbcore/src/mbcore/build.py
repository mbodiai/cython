...
def remove_null_bytes_from_file(filepath: Path) -> bool:
    """
    Remove null bytes from a file using binary mode to avoid encoding issues.
    ...
    try:
        ...
        if b'\0' in content:
            print(f"🧹 Removing null bytes from {filepath}")
            # Remove all null bytes
            cleaned_content = content.replace(b'\0', b'')
            
            # Write back in binary mode first to ensure consistency
            with open(filepath, 'wb') as f:
                f.write(cleaned_content)
                
            # Verify the file doesn't have null bytes
            with open(filepath, 'rb') as f:
                check_content = f.read()
                if b'\0' in check_content:
                    print(f"❌ Failed to remove all null bytes from {filepath}")
                    return False
                    
            print(f"✅ Successfully removed null bytes from {filepath}")
            return True
        else:
            # No null bytes found
            return True
...
def create_clean_stdlib_modules(install_dir: Path) -> None:
    ...
    print("🔧 Creating minimal stub implementations for standard library modules")
...
""")
    ...
    # Create textwrap.py
    with open(packages_dir / "textwrap.py", "w") as f:
        f.write("""# Minimal textwrap stub
def dedent(text):
    return text
""")
    
    # Create warnings.py
    with open(packages_dir / "warnings.py", "w") as f:
        f.write("""# Minimal warnings stub
def warn(*args, **kwargs):
    pass
""")
    
    # Create re.py
    with open(packages_dir / "re.py", "w") as f:
        f.write("""# Minimal re stub
...
""")
    ...
    print("✅ Created minimal stubs for essential modules")
...
def main(
    ...
    # Apply include patterns - only if they're non-empty and explicitly provided
    has_include_pattern = include != "" or include_val
    if has_include_pattern and include_patterns and include_patterns != ['*'] and include_patterns != ['']:
        ...
        print(f"ℹ️ No include patterns specified, keeping all {len(mods)} modules.")
    ...
    if not mods:
        ...
        if not entry_path.exists():
            ...
            # If we have a valid entry point but no modules, add the entry point as a module
            print(f"ℹ️ Using entry point as the only module: {entry_path}")
            mods = [entry_path]
