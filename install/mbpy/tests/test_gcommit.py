...
def test_analyze_diff_add_function(mock_read_text, mock_run_cmd):
    mock_run_cmd.side_effect = ["diff content", "old code", "/mock/repo/root"]
    mock_read_text.return_value = "def new_func(): pass"

    change = analyze_diff("test.py")
    assert change is not None
    assert change.change_type == "modify"
    assert any(ch.startswith("Added function new_func") for ch in change.ast_changes), (
        f"Expected change starting with 'Added function new_func' not found in {change.ast_changes}"
    )


@patch("mbpy.helpers.gcommit.run_cmd")
def test_analyze_diff_with_repo_root(mock_run_cmd):
    mock_run_cmd.side_effect = ["diff content", "old code", "/repo/root"]
...
def test_create_project_item(mock_run_cmd):
    ...
    mock_run_cmd.assert_called_with(
        f'gh project item-create --owner="$GITHUB_OWNER" --project="$GITHUB_PROJECT" --title="Commit Title" --body=\'{body}\''.replace(
            "\\", "\\\\"
        )
    )
