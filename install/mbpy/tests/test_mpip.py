...
# Import cli from main to ensure commands are registered
from mbpy.main import cli
import asyncio
from pathlib import Path
from unittest.mock import patch, MagicMock

# Import the function we need to mock/spy on
from mbpy.pkg.mpip import modify_pyproject


@pytest.mark.asyncio
async def test_add_git_branch_dependency():
    """Tests adding a git dependency with a specific branch."""
    runner = CliRunner()
...
""")
        ...
        git_dep_str = "git+https://github.com/some_org/some_repo.git@my-branch"

        # Mock run_pip_command to simulate success
        async def mock_run_pip_success(*args, **kwargs):
            print(f"[MOCK] run_pip_command called with args: {args}")
            # Simulate successful pip install exit code (0) and return empty output
            # and a dummy parsed_results dict
            from pip._internal.cli.status_codes import SUCCESS

            return SUCCESS, "Successfully installed", {"requirements": ["some-repo"]}

        # Mock modify_dependencies directly to verify it gets called
        # (modify_pyproject is called within modify_dependencies)
        with (
            patch("mbpy.pkg.dependency.run_pip_command", mock_run_pip_success),
            patch(
                "mbpy.pkg.mpip.modify_dependencies", return_value=None
            ) as mock_modify_dependencies,
        ):
            print(f"Commands registered on cli object: {cli.commands.keys()}")
            # Use invoke with await for async command
            result = await asyncio.to_thread(
                runner.invoke, cli, ["add", git_dep_str], catch_exceptions=False
            )
            ...
            # Assert that modify_dependencies was called
            mock_modify_dependencies.assert_called()

            # Optional: Check pyproject.toml content (may need adjustment based on actual formatting)
            # modified_content = pyproject_path.read_text()
            # expected_in_toml = git_dep_str
            # assert expected_in_toml in modified_content, \
            #        f"Expected '{expected_in_toml}' not found in pyproject.toml content:\n{modified_content}"

            # Assert install (run_pip_command mock) was called
            # This requires the original patch target to be inspectable or use a side_effect counter
            # print(f"run_pip_command call count: {mock_run_pip_success.call_count}") # Requires call tracking
