...
class AsyncPExpector(Expecter):
    ...
    def __init__(
        ...
        self._transport: asyncio.SubprocessTransport | None = (
    ...
    async def spawn(self, *, timeout: float | None = None) -> None:  # noqa: ASYNC109
        ...
        try:
            ...
            def protocol_factory():
                return SubprocessStreamProtocol(limit=EXTRA_LIMIT, loop=loop)
            ...
            raise TimeoutError(
                f"Timeout ({connect_timeout}s) while spawning process"
            ) from None
            ...
            async with asyncio.timeout(timeout):
                while True:
                    ...
                    if not chunk:
                        if (
                            self._transport
                            and self._transport.get_returncode() is not None
                        ):
    ...
    async def expect_exact(
        self,
        pattern: str | bytes | List[str | bytes],
        timeout: float | None = None,  # noqa: ASYNC109
    ) -> int:
        ...
        byte_patterns = [
            p.encode(self.encoding, self.codec_errors) if isinstance(p, str) else p
            for p in patterns
        ]
    ...
    async def expect_eof(self, timeout: float | None = None) -> None:
