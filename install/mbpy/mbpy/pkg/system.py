...
class SystemPackageManager:
    ...
    def __init__(self, buildconfig):
        ...
        self._active_tool = self._buildconfig.conf.get(
            ...
            default=self.get_default_tool(),
        )
        self._sudo = self._buildconfig.conf.get(
            "tools.system.package_manager:sudo", default=False, check_type=bool
        )
        ...
        self._mode = self._buildconfig.conf.get(
            "tools.system.package_manager:mode", default=self.mode_check
        )
        self._arch = self._buildconfig.settings_build.get_safe("arch")
        self._arch_names = {}
    ...
    def get_default_tool(self):
        ...
        if os_name in ["Linux", "FreeBSD"]:
            ...
            os_name == "Windows"
            and self._buildconfig.settings.get_safe("os.subsystem") == "msys2"
        ):
            os_name = "msys2"
        manager_mapping = {
            "apt-get": [
    ...
    def _install_substitutes(
        self, *packages_substitutes, update=False, check=True, **kwargs
    ):
        errors = []
        ...
        raise RuntimeError(
            "None of the installs for the package substitutes succeeded."
        )
    ...
    def _install(self, packages, update=False, check=True, host_package=True, **kwargs):
        ...
        self._buildconfig.output.info(
            "System requirements: {} already installed".format(" ".join(packages))
        )
        return None
    ...
    def _update(self):
        ...
        if self._mode == self.mode_install:
            command = self.update_command.format(
                sudo=self.sudo_str, tool=self.tool_name
            )
    ...
    def _check(self, packages, host_package=True):
        return [
            pkg
            for pkg in packages
            if self.check_package(self.get_package_name(pkg, host_package=host_package))
            != 0
        ]
...
class Apt(SystemPackageManager):
    ...
    def install(
        self, packages, update=False, check=True, recommends=False, host_package=True
    ):
...
class Chocolatey(SystemPackageManager):
    ...
    check_command = (
        "{tool} search --local-only --exact {package} | "
...
if __name__ == "__main__":
    ...
    def build_languages(config: BuildConfig) -> None:
        with Progress() as progress:
            ...
            out_lib = out_lib / (
                "languages.dll" if sys.platform == "win32" else "languages.so"
            )
