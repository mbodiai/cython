...
from mbcore.display import safe_print, smart_import
from typing_extensions import TYPE_CHECKING


def add_auto_cpdef_to_package(
    package_path: Path | str, outdir="./tmp", progress=None, force=False, cythonize=True
):
    ...
    for f in package_path.glob("**/*.py"):
        ...
        try:
            ...
            with redirect_stderr(stderr), redirect_stdout(stdout):
                try:
                    _cythonize(
                        [str(out_file)], force=True, quiet=False, show_all_warnings=True
                    )
