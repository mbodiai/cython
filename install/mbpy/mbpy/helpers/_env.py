...
from typing_extensions import NamedTuple, TYPE_CHECKING
from mbcore._traceback import install

if TYPE_CHECKING:
    from mbpy.env import Env
install(show_locals=bool(os.getenv("SHOW_LOCALS", False)))
...
def detect_conda_env_interpreter() -> "Env":
    import os
    from mbpy.env import Env
    ...
    env_var = os.environ.get("CONDA_DEFAULT_ENV")
    exe = os.getenv("CONDA_PYTHON_EXE")
    exe = Path(exe) if exe is not None else None
    if not env_var or not exe:
        return None

    return Env(type="conda", name=env_var, python=exe)
...
class version_info(NamedTuple):  # noqa
    major: int = 0
    minor: int = 0
    micro: int = 0
    releaselevel: str | int = ""

    def __bool__(self):
        return bool(self.major or self.minor or self.micro or self.releaselevel)

    def __str__(self):
        if not any((self.major, self.minor, self.micro, self.releaselevel)):
            return ""
        return f"{self.major}.{self.minor}.{self.micro}{'.' + str(self.releaselevel) if self.releaselevel else ''}"

    def __repr__(self):
        return f"{self.major}.{self.minor}.{self.micro}{'.' + str(self.releaselevel) if self.releaselevel else ''}"

    def __le__(self, other: "version_info | tuple") -> bool:
        other = version_info(*other) if not isinstance(other, version_info) else other
    ...
    def __lt__(self, other: "version_info") -> bool:
        if self.major < other.major:
            return True
        if self.major == other.major and self.minor < other.minor:
            return True
    ...
    def __ge__(self, other: "version_info") -> bool:
        if self.major >= other.major:
            return True
        if self.major == other.major and self.minor >= other.minor:
            return True
    ...
    def __gt__(self, other: "version_info") -> bool:
        if self.major > other.major:
            return True
        if self.major == other.major and self.minor > other.minor:
            return True
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, version_info):
            return False
        return (
            self.major == other.major
            and self.minor == other.minor
...
def iter_modules(path=None, prefix=""):
    ...
    if path is None:
        ...
        raise ValueError(
            f"path must be None or list of paths to look for modules in. Got {path}"
        )
    else:
...
def _iter_file_finder_modules(importer: "importlib.machinery.FileFinder", prefix=""):
...
def iter_importers(fullname=""):
    ...
    """
    if "test" in fullname or "site-packages" in fullname:
        return
    ...
    if "." in fullname:
        ...
        path = sys.path
    for item in path:
        if "test" in item or "site-packages" in item:
            continue
...
def get_loader(module_or_name):
    ...
    warnings._deprecated(
        "pkgutil.get_loader",
        f"{warnings._DEPRECATED_MSG}; use importlib.util.find_spec() instead",
        remove=(3, 14),
    )
...
def isverbose():
    return any(s in sys.flags for s in ("-v", "v", "vv", "vvv", "-d", "-debug", "-dd"))


def resolve_name(name: str):
    """Resolve a fully-qualified name into an object, never raising unhandled exceptions.

    On success: returns the resolved object.
    ...
    base_module = _safe_import_module(base_name)
    if not base_module:
        _log_error(f"Base module '{base_name}' not found or failed to import.")
        return None

    resolved_obj = base_module
    for attr in parts[1:]:
        # Attempt to get attribute
        if hasattr(resolved_obj, attr):
            resolved_obj = getattr(resolved_obj, attr)
            continue

        # Attempt submodule import
        if isinstance(resolved_obj, ModuleType):
            maybe_submodule = _safe_import_module(f"{resolved_obj.__name__}.{attr}")
            if maybe_submodule:
                resolved_obj = maybe_submodule
                continue

        _log_error(
            f"'{resolved_obj.__name__}' has no attribute or submodule '{attr}'. "
            "Likely not initialized or does not exist."
        )
        return None
...
def _safe_import_module(mod_name: str):
    ...
    try:
        return importlib.import_module(mod_name.split(".")[-1], mod_name.split(".")[0])
    except ImportError:
...
def _log_error(msg: str):
    ...
    logging.getLogger("rich").error(f"[resolve_name error]: {msg}")
...
def _sys_version(sys_version=None):
    ...
    if sys.platform.startswith("java"):
        ...
        if match is None:
            raise ValueError(
                f"failed to parse CPython sys.version: {repr(sys_version)}"
            )
...
def platform(aliased=False, terse=False):
    ...
    if system == "Windows":
        ...
        platform = (
            _platform(system, release)
            if terse
            else _platform(system, release, version, csd)
        )
