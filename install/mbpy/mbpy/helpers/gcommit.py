...
def analyze_diff(file: str, repo_root: str | None = None) -> CodeChange | None:
    ...
    def get_node_changes(new_node: ast.AST) -> List[str]:
        ...
        if isinstance(new_node, ast.FunctionDef):
            ...
            return [
                f"Added function {new_node.name}({', '.join(args)}) at {get_object_ref(new_node)}"
            ]
        if isinstance(new_node, ast.ClassDef):
            bases = [b.id for b in new_node.bases if isinstance(b, ast.Name)]
            return [
                f"Added class {new_node.name}({', '.join(bases)}) at {get_object_ref(new_node)}"
            ]
        if isinstance(new_node, ast.Import):
            return [
                f"Added import {n.name} at {get_object_ref(new_node)}"
                for n in new_node.names
            ]
        return []
...
def create_project_item(title: str, body: str) -> str:
    ...
    # Ensure body is properly escaped for shell command
    escaped_body = shlex.quote(body)
    cmd = f'gh project item-create --owner="$GITHUB_OWNER" --project="$GITHUB_PROJECT" --title="{title}" --body={escaped_body}'
    return run_cmd(cmd)


@click.command()
@click.option(
    "--analyze-only", is_flag=True, help="Only analyze changes without committing"
)
...
def main(analyze_only: bool, create_item: bool, repo_path: str | None = None) -> None:
    ...
    if create_item and result:
        ...
        create_project_item(message, body)
