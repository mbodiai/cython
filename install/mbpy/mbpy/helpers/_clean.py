...
@click.option(
    "--build-lib", default="build/lib", help="Build directory for all modules"
)
@click.option(
    "--build-temp", "-t", default="build/temp", help="Temporary build directory"
)
@click.option(
    "--build-scripts", default="build/scripts", help="Build directory for scripts"
)
@click.option(
    "--bdist-base", default="bdist", help="Temporary directory for built distributions"
)
@click.option(
    "--all",
    "-a",
    is_flag=True,
    help="Remove all build output, including egg and .so files",
)
    ...
    if all:
        ...
        for root, _dirs, files in os.walk("."):
            for file in files:
                ...
                if Path(file).with_suffix(".py").exists() and Path(file).suffix in (
                    ".c",
                    ".cpp",
                    ".h",
                ):
