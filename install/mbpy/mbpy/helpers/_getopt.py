...
# Add this function to handle ANSI color wrapping
def colorize(text, color=None, bold=False):
    """Wrap text in ANSI color codes if color is specified."""
...
# Modify the print_help method in the GetOpt class
def print_help(self, header=None, file=None, color=True):
    """Print help text with optional ANSI colors."""
    if file is None:
        file = sys.stdout
    ...
    for line in self.generate_help(header):
        if color and line.startswith("  --"):
            ...
            if len(parts) >= 2:
                ...
                line = f"  {colorize(option, 'green', bold=True)}  {desc}"
        elif color and (header and line == header):
            # Colorize header
            line = colorize(line, "cyan", bold=True)

        file.write(line + "\n")


class GetoptError(Exception):
    """Exception raised by getopt() when it finds an invalid option."""
...
class ArgError(Exception):
    """Raised when an error occurs while parsing command-line options."""
...
class GetOpt:
    ...
    def _build_index(self):
        ...
        for option in self.option_table or []:
    ...
    def add_option(self, long_option, short_option=None, help_string=None):
        if long_option in self.option_index:
            raise GetoptError(f"option conflict: already an option '{long_option}'")
    ...
    def generate_help(self, header=None):
        ...
        output) from the option table for this Getopt object.
        """
...
say, "How should I know?"].)"""
    # Get the terminal width without using environment variables
