#!/usr/bin/env python3
r"""This file compares the output and runtime of running normal vs incremental mode
...
def get_commits_starting_at(
    repo_folder_path: str, start_commit: str
) -> list[tuple[str, str]]:
...
def set_expected(
    ...
    """  # noqa: D401
    for commit_id, _message in commits:
...
def main() -> None:
    def help_factory(prog):
        return RawDescriptionHelpFormatter(prog=prog, max_help_position=32)
    ...
    if params.file_path:  # noqa: SIM108
        target_file_path = os.path.abspath(
            os.path.join(temp_repo_path, params.file_path)
        )
    else:
