...
class DataclassTransformer:
    ...
    def _add_dunder_replace(self, attributes: list[DataclassAttribute]) -> None:
        ...
        type_vars = list(self._cls.type_vars)
        add_method_to_class(
            self._api,
            self._cls,
            "__replace__",
    ...
    def _collect_field_args(
        ...
        if (
            ...
            args = {}
            for name, arg, kind in zip(
                expr.arg_names, expr.args, expr.arg_kinds, strict=False
            ):
...
def _meet_replace_sigs(sigs: list[CallableType]) -> CallableType:
    ...
    args = {
        ...
        for name, typ, kind in zip(
            sigs[0].arg_names, sigs[0].arg_types, sigs[0].arg_kinds, strict=False
        )
    }
    ...
    for sig in sigs[1:]:
        sig_args = {
            name: (typ, kind)
            for name, typ, kind in zip(
                sig.arg_names, sig.arg_types, sig.arg_kinds, strict=False
            )
        }
