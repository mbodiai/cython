"""Converter from CHANGELOG.md (Markdown) to HTML suitable for a mbpy blog post.

How to use:

1. Write release notes in CHANGELOG.md.
2. Make sure the heading for the next release is of form `## mbpy X.Y`.
...
def format_code(h: str) -> str:
    ...
    while i < len(a):
        if a[i].startswith("    ") or a[i].startswith("```"):
            ...
            r.append("<pre>")
            while i < len(a) and (
                (indent and a[i].startswith("    "))
                or (not indent and not a[i].startswith("```"))
            ):
                # Undo &gt; and &lt;
...
def convert(src: str) -> str:
    ...
    # Title
    h = re.sub(r"^## (mbpy [0-9.]+)", r"<h1>\1 Released</h1>", h, flags=re.MULTILINE)
    ...
    h = re.sub(
        r"\[(#[0-9]+)\]\(https://github.com/python/mbpy/pull/[0-9]+/?\)", r"\1", h
    )
    ...
    h = re.sub(
        r"fixes #([0-9]+)",
        r'fixes issue <a href="https://github.com/python/mbpy/issues/\1">\1</a>',
        h,
    )
    h = re.sub(
        r"#([0-9]+)", r'PR <a href="https://github.com/python/mbpy/pull/\1">\1</a>', h
    )
    h = re.sub(r"\) \(PR", ", PR", h)
...
def main() -> None:
    ...
    parser.add_argument("version", help="mbpy version, in form X.Y or X.Y.Z")
    args = parser.parse_args()
    version: str = args.version
    if not re.match(r"[0-9]+(\.[0-9]+)+$", version):
        sys.exit(f"error: Version must be of form X.Y or X.Y.Z, not {version!r}")
    changelog_path = os.path.join(
        os.path.dirname(__file__), os.path.pardir, "CHANGELOG.md"
    )
