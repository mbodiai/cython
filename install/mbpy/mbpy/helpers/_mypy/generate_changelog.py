"""Generate the changelog for a mbpy release."""
...
def find_all_release_branches() -> list[tuple[int, int]]:
    result = subprocess.run(
        ["git", "branch", "-r"], text=True, capture_output=True, check=True
    )
    versions = []
...
def find_changes_between_releases(old_branch: str, new_branch: str) -> list[CommitInfo]:
    ...
    return filter_out_commits_from_old_release_branch(new_commits, old_commits)
...
def format_changelog_entry(c: CommitInfo) -> str:
    ...
    if c.pr_number:
        s += f", PR [{c.pr_number}](https://github.com/python/mbpy/pull/{c.pr_number})"
    s += ")"
...
def main() -> None:
    ...
    parser.add_argument("version", help="target mbpy version (form X.Y)")
