...
def _make_rich_rext(
    text: Union[str, Text], style: StyleType, formatter: RichHelpFormatter
) -> Union[Markdown, Text]:
    """
...
def _get_help_text(
    ...
    yield _make_rich_rext(
        first_line.strip(), config.style_helptext_first_line, formatter
    )
...
def _get_option_help(
    ...
    if not hasattr(param, "show_default"):
        ...
        parse_default = bool(
            ...
            and (param.show_default or getattr(ctx, "show_default", None))
        )
        ...
        parse_default = bool(
            show_default_is_str or (show_default and (param.default is not None))
        )
    ...
    if param.required:
        items.append(
            Text(config.required_long_string, style=config.style_required_long)
        )
...
def _make_command_help(
    help_text: str, formatter: RichHelpFormatter, is_deprecated: bool
) -> Union[Text, Markdown]:
    """
    ...
    """
    formatter.config.use_markdown = False
    ...
    renderable = _make_rich_rext(
        help_text, formatter.config.style_option_help, formatter
    )
    return renderable
...
def get_rich_help_text(
    self: Command, ctx: click.Context, formatter: RichHelpFormatter
) -> None:
...
def _resolve_groups(
    ctx: click.Context,
    groups: Dict[str, List[OptionGroupDict]],
    group_attribute: Literal["options"],
...
def _resolve_groups(
    ctx: click.Context,
    groups: Dict[str, List[CommandGroupDict]],
    group_attribute: Literal["commands"],
...
def _resolve_groups(
    ...
    group_attribute: Literal["commands", "options"],
) -> List[GroupType]:
...
def get_rich_options(
    ...
    for param in obj.get_params(ctx):
        ...
        # No break, no mention - add to the default group
            ...
            if (
                isinstance(param, click.core.Argument)
                and not formatter.config.group_arguments_options
            ):
    ...
    if len(argument_group_options) > 0:
        for grp in option_groups:
            if grp.get(
                "name", ""
            ) == formatter.config.arguments_panel_title and not grp.get("options"):
...
def get_rich_commands(
    ...
    for cmd_group in cmd_groups:
        ...
        if formatter.config.style_commands_table_column_width_ratio is None:
            table_column_width_ratio: Union[Tuple[None, None], Tuple[int, int]] = (
                None,
                None,
            )
        else:
    ...
    if self.epilog:
        ...
        formatter.write(
            Padding(
                Align(
                    _make_rich_rext(
                        epilog, formatter.config.style_epilog_text, formatter
                    ),
                    pad=False,
                ),
                1,
        ...
        formatter.write(
            Padding(
                _make_rich_rext(
                    ...
                    formatter.config.style_footer_text,
                    formatter,
                ),
                (1, 1, 0, 1),
            )
