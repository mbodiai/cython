...
async def maybetouch(
    ...
    existing_content: Literal["merge", "replace", "forbid", "leave"] = "merge",
) -> Path | None:
    ...
    if content and existing_content == "merge":
        content = "\n".join(
            [*Path(path).read_text().splitlines(), *content.splitlines()]
        )
...
def walk(
    node: T,
    ignore_pred: str | Callable[[T], bool] = IGNORE_FILES,
    max_depth=-1,
    depth=0,
):
    yield from filterfalse(
        ignore_pred, walk_bfs(node, depth=depth, max_depth=max_depth)
    )
...
async def setup_documentation(
    ...
    if not project_root:
        raise FileNotFoundError(
            f"Could not find project root {search_window} directories up or down."
        )
    console.print(
        f"Using project root: {project_root}...", style="bold light_goldenrod2"
    )
    docs_dir = project_root / "docs"
    docs_dir.mkdir(exist_ok=True, parents=True)
    if docs_dir.exists() and len(list(docs_dir.iterdir())) > 0:
        resp = prompt_ask(
            prompt=f"Documentation directory: {docs_dir} is not empty. Do you want to continue?"
        )
        if not resp:
...
def extract_docstrings(project_path) -> dict[str, dict[str, str]]:
    ...
    for py_file in project_path.rglob(f"*.py | !{IGNORE_FILES} | !*//.*"):
        with Path(py_file).open() as f:
            tree: Any = None
            with ctx.suppress.logignore(
                SyntaxError, UnicodeError, FileNotFoundError
            ) as e:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef | ast.ClassDef | ast.Module):
                    name = (
                        node.name
                        if not isinstance(node, ModuleType | ast.Module)
                        else node.__name__
                    )
                    signature = ""
...
---
"""
            api_content += obj_api.format(
                module_name=module_name, obj_name=obj_name, docstring=docstring
            )
    if api_content:
...
async def create_pyproject_toml(
    ...
    *,
    ...
    if (project_path / "pyproject.toml").exists() and not prompt_ask(
        f"project.toml already exists in dir: {project_path}. Do you want to continue? (y/n)"
    ):
        console.print("Exiting...", style="bold blue")
        return None
    try:
    ...
    if manager in ("hatch", "uv"):
        tool.setdefault("hatchling", tomlkit.table()).setdefault(
            "build", tomlkit.table()
        ).setdefault(
            "hooks",
            tomlkit.table(),
        ).setdefault("build-scripts", tomlkit.table()).setdefault(
