    ...
    def core(self) -> CoreMetadata:
        if self._core is None:
            metadata = CoreMetadata(
                self.root, self.core_raw_metadata, self.hatch.metadata, self.context
            )
            ...
            if metadata_hooks:
                ...
                if metadata.dynamic:
                    for metadata_hook in metadata_hooks.values():
                        ...
                        metadata.add_known_classifiers(
                            metadata_hook.get_known_classifiers()
                        )
...
class CoreMetadata:
    ...
    def __init__(
        ...
        self._optional_dependencies_complex: (
            dict[str, dict[str, Requirement]] | None
        ) = None
        ...
        if self._readme is None:
            ...
            if readme is None:
                ...
                if content_type is None:
                    message = (
                        "Field `content-type` is required in the `project.readme` table"
                    )
                    raise ValueError(message)
    ...
    def license(self) -> str:
        ...
        if self._license is None:
            ...
            if data is None:
                ...
                try:
                    self._license_expression = str(
                        canonicalize_license_expression(data)
                    )
        ...
        if self._license_files is None:
            ...
            for i, pattern in enumerate(globs, 1):
                if not isinstance(pattern, str):
                    message = (
                        f"Entry #{i} of field `project.license-files` must be a string"
                    )
                    raise TypeError(message)
    ...
    def authors(self) -> list[str]:
        ...
        if self._authors is None:
            ...
            for i, data in enumerate(authors, 1):
                ...
                if name and email:
                    authors_data["email"].append(
                        str(Address(display_name=name, addr_spec=email))
                    )
                elif email:
        ...
        if self._keywords is None:
            ...
            for i, keyword in enumerate(keywords, 1):
                if not isinstance(keyword, str):
                    message = (
                        f"Keyword #{i} of field `project.keywords` must be a string"
                    )
                    raise TypeError(message)
    ...
    def classifiers(self) -> list[str]:
        ...
        if self._classifiers is None:
            ...
            verify_classifiers = not os.environ.get(
            ...
            if verify_classifiers:
                ...
                known_classifiers = (
                    trove_classifiers.classifiers | self._extra_classifiers
                )
                sorted_classifiers = list(trove_classifiers.sorted_classifiers)

                for classifier in sorted(
                    self._extra_classifiers - trove_classifiers.classifiers
                ):
            ...
            if not verify_classifiers:
                ...
                sorted_classifiers = sorted(
                    classifiers,
                    key=lambda value: (
                        [(a, int(b) if b else None) for a, b in split_re.findall(value)]
                    ),
                )
            ...
            self._classifiers = sorted(
                unique_classifiers,
                key=lambda c: -1
                if self.__classifier_is_private(c)
                else sorted_classifiers.index(c),
            )
        ...
        if self._urls is None:
            if "urls" in self.config:
                ...
                if "urls" in self.dynamic:
                    message = "Metadata field `urls` cannot be both statically defined and listed in field `project.dynamic`"
                    raise ValueError(message)
            else:
                urls = {}
    ...
    def entry_points(self) -> dict[str, dict[str, str]]:
        ...
        if self._entry_points is None:
            ...
            for forbidden_field, expected_field in (
        ...
        if self._dependencies_complex is None:
            ...
            for i, entry in enumerate(dependencies, 1):
                ...
                try:
                    ...
                    if (
                        requirement.url
                        and not self.hatch_metadata.allow_direct_references
                    ):
                        message = (
    ...
    def optional_dependencies_complex(self) -> dict[str, dict[str, Requirement]]:
        ...
        if self._optional_dependencies_complex is None:
            ...
            for option, dependencies in optional_dependencies.items():
                ...
                if not isinstance(dependencies, list):
                    message = f"Dependencies for option `{option}` of field `project.optional-dependencies` must be an array"
                    raise TypeError(message)
                    ...
                    try:
                        ...
                        if requirement.name == self.name:
                            if normalized_option in inherited_options:
                                inherited_options[normalized_option].update(
                                    requirement.extras
                                )
                            else:
                                inherited_options[normalized_option] = set(
                                    requirement.extras
                                )
                        else:
            ...
            self._optional_dependencies_complex = {
                option: dict(sorted(entries.items()))
                for option, entries in sorted(optional_dependency_entries.items())
            }
    ...
    def optional_dependencies(self) -> dict[str, list[str]]:
        ...
        if self._optional_dependencies is None:
            self._optional_dependencies = {
                option: list(entries)
                for option, entries in self.optional_dependencies_complex.items()
            }
...
class HatchMetadata(Generic[PluginManagerBound]):
    ...
    def metadata(self) -> HatchMetadataSettings:
        if self._metadata is None:
            ...
            self._metadata = HatchMetadataSettings(
                self.root, metadata_config, self.plugin_manager
            )
    ...
    def version(self) -> HatchVersionConfig:
        if self._version is None:
            ...
            self._version = HatchVersionConfig(
                self.root, deepcopy(options), self.plugin_manager
            )
...
class HatchVersionConfig(Generic[PluginManagerBound]):
    def __init__(
        self, root: str, config: dict[str, Any], plugin_manager: PluginManagerBound
    ) -> None:
        self.root = root
        self.config = config
        self.plugin_manager = plugin_manager
...
class HatchMetadataSettings(Generic[PluginManagerBound]):
    ...
    def allow_direct_references(self) -> bool:
        if self._allow_direct_references is None:
            allow_direct_references: bool = self.config.get(
    ...
    def allow_ambiguous_features(self) -> bool:
        ...
        if self._allow_ambiguous_features is None:
            allow_ambiguous_features: bool = self.config.get(
    ...
    if dependent_option in inherited_options:
        for selected_option in inherited_options[dependent_option]:
            ...
            if selected_option not in optional_dependencies_complex:
                message = f"Unknown recursive dependency group in field `project.optional-dependencies`: {selected_option}"
                raise ValueError(message)

            optional_dependencies_complex[dependent_option].update(
                optional_dependencies_complex[selected_option]
            )
