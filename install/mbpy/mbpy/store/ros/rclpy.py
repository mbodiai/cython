...
def get_type_info(typ: Type, context: SchemaContext) -> Dict[str, Any]:
    ...
    if origin is not None:
        ...
        return {
            "type": str(origin),
            "arguments": [get_type_info(arg, context) for arg in args],
        }
...
def get_message_info(msg_type, context: SchemaContext) -> Dict[str, Any]:
    ...
    return {
        ...
        "fields": {
            field: {
                "type": get_type_info(field_type, context),
                "default": getattr(msg_type(), field),
            }
            for field, field_type in fields_and_types.items()
        },
        "constants": {
            name: value for name, value in msg_type.__dict__.items() if name.isupper()
        },
    }
...
def get_class_info(cls: Type, context: SchemaContext) -> Dict[str, Any]:
    ...
    try:
        ...
        class_info["type_hints"] = {
            name: get_type_info(typ, context) for name, typ in hints.items()
        }
    except Exception:
        pass
    ...
    for name, member in inspect.getmembers(cls):
        ...
        if inspect.isfunction(member) or inspect.ismethod(member):
            ...
            class_info["properties"][name] = {
                "doc": member.__doc__,
                "type": get_type_info(
                    member.fget.__annotations__.get("return", Any), context
                )
