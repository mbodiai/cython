...
class Preprocessor:
    def __init__(self, lexer=None):
        ...
        tm = time.localtime()
        self.define('__DATE__ "{}"'.format(time.strftime("%b %d %Y", tm)))
        self.define('__TIME__ "{}"'.format(time.strftime("%H:%M:%S", tm)))
        self.parser = None
        ...
        for argnum, i in macro.str_patch:
            if argnum not in str_expansion:
                str_expansion[argnum] = (
                    '"{}"'.format("".join([x.value for x in args[argnum]]))
                ).replace("\\", "\\\\")
    ...
    # Parse input text.
    # ----------------------------------------------------------------------
    def parse(self, input, source=None, ignore=None) -> None:
        if ignore is None:
            ignore = {}
        self.ignore = ignore
