...
clean_artifacts() {
    ...
    find "$SEARCH_PATH" "${find_args[@]}" ! -path '*/.venv/*' ! -path '*/site-packages/*' | while read -r file; do
        # Make case matching sensitive
        shopt -u nocasematch
        case "$file" in
            *venv*|*site-packages*)
                continue
                ;;
            # Be more specific with patterns to prevent accidental matches with different cases
            *.pyc|*.pyo|*/__pycache__|*.tmp|*.log|*.bak|*.swp|*.DS_Store|*/dist|*/*.egg-info|*/*.dist-info|*.o|*.so|*/tmp|*/bak|*/swp|*/.mypy_cache|*/.pytest_cache|*/.ruff_cache|*/.python-version)
                ;;
            */build)
                if [ -z "$FORCE_CLEAN" ]; then
                    # Check if only build artifacts are present.
                    for file in $(find "$file" -type f); do
                        case "$file" in
                            *.pyc|*.pyo|*__pycache__|*.tmp|*.log|*.bak|*.swp|*.DS_Store|*.mypy_cache|*.pytest_cache|*.ruff_cache|*.python-version|*.egg-info|*.dist-info|*.o|*.so|*.exe|*.dll|*.lib|*.obj|*.pdb|*.ilk|*.exp|*.manifest)
                                continue
                                ;;
                            *)
                                if [ -z "$FORCE_CLEAN" ]; then
                    ...
                    if [ -n "$DRY_RUN" ]; then
                        ...
                        echo "Skipping directory: $file. Pass -a to clean build artifacts."
                    else
    ...
    if [ -f "$skipped_file" ] && [ -s "$skipped_file" ]; then
        echo "Build directory contained non-build artifacts:"
        cat "$skipped_file"
        echo "Pass -f to force clean."
    fi
    if [ -f "$temp_status" ]; then
        rm -f "$temp_status"
...
echo "Cleaning artifacts in: $SEARCH_PATH for pattern: $CUSTOM_PATTERN with build: $CLEAN_BUILD and force: $FORCE_CLEAN"
clean_artifacts
